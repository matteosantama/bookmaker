# SAME AS PREPAREDATA.py
# PYTHON 3 Compatible Version used by Bodhi
import argparse
import os
...
raptor_ofeatures = ['raptor_offense',
                   'raptor_onoff_offense',
                   'war_total',
                   'pace_impact']
raptor_dfeatures = [ 'raptor_defense',
...
def process_and_write(
    ...
    if raptor:
        ...
        raptordf = pd.read_csv(raptor_path, index_col=0)  
        livestats = __prepare_raptor(raptordf, boxscores, livestats)
        if verbose:
    ...
    for i, (gameid, gamedf) in enumerate(schedule.groupby(level='GAME_ID'), 1):
        ...
        if (games_played_X > cutoff and games_played_Y > cutoff):
            ...
            this_raptor_X = pd.concat(
                    [stats_series_X[raptor_ofeatures]], keys=['this_raptor'])
            other_raptor_X = pd.concat(
                    [stats_series_Y[raptor_dfeatures]], keys=['other_raptor'])
            ...
            this_raptor_Y = pd.concat(
                    [stats_series_Y[raptor_ofeatures]], keys=['this_raptor'])
            other_raptor_Y = pd.concat(
                    [stats_series_X[raptor_dfeatures]], keys=['other_raptor'])
